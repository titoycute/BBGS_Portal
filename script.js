    ...
    this.templates = {
        ...
        dashboard: () => {
            ...
            return `
                ...
                <button onclick="app.navigateTo('qrSpots')"
            class="bg-gray-700 p-4 rounded-xl flex flex-col items-center justify-center space-y-2 hover:bg-gray-600 transition-colors"><i
                data-lucide="map-pin" class="text-green-400"></i><span class="font-semibold">QR Spots</span></button>
                <button  class="bg-gray-700 p-4 rounded-xl flex flex-col items-center justify-center space-y-2 hover:bg-gray-600 transition-colors opacity-40">
                    <i data-lucide="gamepad-2" class="text-purple-400"></i>
                    <span class="font-semibold">Games</span>
                </button> 
                <button  class="bg-gray-700 p-4 rounded-xl flex flex-col items-center justify-center space-y-2 hover:bg-gray-600 transition-colors opacity-40">
                    <i data-lucide="messages-square" class="text-pink-400"></i>
        ...
        about: () => `
            ...
            ` : ''}
            <h2 class="text-2xl font-bold text-center mb-6">About Pride Pass</h2>
            <div class="space-y-8 bg-gray-900/50 p-6 rounded-xl">
                
                <!-- Section 1: What is Pride Pass? -->
                <div  style="text-align: justify;">
                    ...
                    <p class="text-gray-300" style="text-indent: 30px;">
                        The Pride Pass, a pioneering digital initiative by the Biliran Bisexual and Gay Society, is a powerful tool for LGBTQIAP+ individuals and allies alike. It serves as your all-in-one community pass, leveraging an innovative QR code system to enhance connectivity, streamline event check-ins, and facilitate seamless integration within the vibrant Biliran community. Embrace inclusivity, embrace empowerment, embrace the future with the Pride Pass.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <div class="bg-gray-800 p-4 rounded-lg flex items-center space-x-4">
                        <img src="https://scontent.fceb1-3.fna.fbcdn.net/v/t39.30808-1/484884415_1907209106477957_3565712896536834326_n.jpg?stp=c0.0.1181.1181a_dst-jpg_s200x200_tt6&_nc_cat=103&ccb=1-7&_nc_sid=1d2534&_nc_eui2=AeE7Yioq1vGaHK5XvrWIWI22YPEh_0Btsetg8SH_QG2x61J4ybJfTbXKL3nyD266qVv_IfyEAsoI3y9OHZyUT8yU&_nc_ohc=fOFN7AW0VeUQ7kNvwEDEYBy&_nc_oc=AdmD6_Iadgfwrmir6mmK1CA7-_aztFSkX7dg9Iest5C0rYGFZr2HKvXapohHqNrLsSU&_nc_zt=24&_nc_ht=scontent.fceb1-3.fna&_nc_gid=aAvOi_1LweAYFNSRrhOpzQ&oh=00_AfWCadjZH6Yvjp1zryIrHCxHFedM_cCsNr2gIqR4hz1Igg&oe=68B1B7C3" class="w-16 h-16 rounded-full object-cover border-2 border-gray-600">
                        <div>
        ...
        profile: () => {
            ...
            <div class="bg-gray-900/50 p-4 rounded-lg mb-4 text-center">
                ...
                ${user.isValidated ? `<p class="text-green-400 font-bold">✓ Verified on ${user.validatedAt || ''}</p>` : '<p class="text-amber-400 font-bold">Pending Approval</p>'}
            </div>
                  ...
                  <div><label class="text-sm text-gray-400">Email</label><input name="email" value="${user.email}" class="w-full bg-gray-600 rounded-lg p-3 mt-1" readonly></div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-4 gap-3">
                            ${this.renderThemeOption('default', 'Default', 'bg-gray-800', user.theme)}
                            ${this.renderThemeOption('light', 'Golden', 'bg-yellow-200', user.theme)}
                            ${this.renderThemeOption('blue', 'Blue', 'bg-blue-600', user.theme)}
                            ${this.renderThemeOption('green', 'Green', 'bg-green-600', user.theme)}
                            ${this.renderThemeOption('pink', 'Pink', 'bg-pink-500', user.theme)}
                            ${this.renderThemeOption('rainbow2', 'Linear', 'pride-gradient-bg', user.theme)}
                            ${this.renderThemeOption('rainbow', 'Digital', 'bg-black', user.theme)}
                        </div>
                    </div>
                </div>
            </form>
        ...
        admin: () => {
            ...
            const filteredRewards = this.state.adminRewards.filter(r => r.name && typeof r.name === 'string' && r.name.toLowerCase().includes(this.state.adminRewardSearch)); const filteredMembers = this.state.users.filter(u => u.isValidated && u.email !== this.state.adminEmail && (u.firstName + ' ' + u.lastName).toLowerCase().includes(this.state.adminMemberSearch));
...
<div id="rewards-list" class="space-y-2">${filteredRewards.map(reward => `<div class="bg-gray-700 p-3 rounded-lg"><div class="flex items-center justify-between"><div><p class="font-semibold">${reward.name} <span class="text-xs ml-2 px-2 py-0.5 rounded-full ${reward.isVisible ? 'bg-green-500/20 text-green-400' : 'bg-gray-600 text-gray-400'}">${reward.isVisible ? 'Visible' : 'Hidden'}</span></p><p class="text-sm ${reward.type === 'cost' ? 'text-red-400' : 'text-green-400'}">${reward.type === 'cost' ? '-' : '+'}${reward.cost} Points</p></div><div class="flex items-center space-x-2"><div class="bg-white p-1 rounded-md cursor-pointer" onclick="app.openRewardQrModal('${reward.id}')"><canvas id="reward-qr-${reward.id}" class="reward-qr-canvas" data-reward-id="${reward.id}"></canvas></div><button onclick="app.switchToEditRewardMode('${reward.id}'); document.querySelector('[data-tab=rewards]').click();" class="p-2 bg-gray-600 rounded-md hover:bg-gray-500"><i data-lucide="edit-2" class="w-4 h-4"></i></button><button onclick="app.handleAdminDeleteReward('${reward.id}')" class="p-2 bg-red-900/50 rounded-md hover:bg-red-900/80"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div></div></div>`).join('') || '<p class="text-gray-400 text-center">No rewards found.</p>'}</div>
    ...
    this.attachListeners = () => {
        ...
        const mapSpotsListener = onSnapshot(query(collection(this.fb.db, this.paths.mapSpots)), (snapshot) => {
            this.state.mapSpots = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    this.initUserMap = () => {
        ...
        // Coordinates for Catarman, Eastern Visayas
        const map = L.map('user-map').setView([11.5833, 124.4333], 13);
    ...
    this.initAdminMap = () => {
        ...
        const map = L.map('admin-map').setView([11.5833, 124.4333], 13);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

        let newSpotMarker;
        map.on('click', (e) => {
            const { lat, lng } = e.latlng;
            document.querySelector('#add-spot-form input[name="spotLat"]').value = lat;
            document.querySelector('#add-spot-form input[name="spotLng"]').value = lng;
            document.getElementById('coords-display').textContent = `${lat.toFixed(6)}, ${lng.toFixed(6)}`;

            if (newSpotMarker) {
                newSpotMarker.setLatLng(e.latlng);
            } else {
                newSpotMarker = L.marker(e.latlng, { draggable: true }).addTo(map);
                newSpotMarker.on('dragend', (event) => {
                    const marker = event.target;
                    const position = marker.getLatLng();
                    document.querySelector('#add-spot-form input[name="spotLat"]').value = position.lat;
                    document.querySelector('#add-spot-form input[name="spotLng"]').value = position.lng;
                    document.getElementById('coords-display').textContent = `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`;
                });
            }
        });
    ...
    this.populateAdminSpotForm = () => {
        const typeSelect = document.querySelector('#add-spot-form select[name="spotType"]');
        const itemSelect = document.querySelector('#add-spot-form select[name="linkedId"]');
        ...
        if (selectedType === 'event') {
            this.state.events.forEach(item => options += `<option value="${item.id}">${item.name}</option>`);
        } else if (selectedType === 'reward') {
            this.state.adminRewards.forEach(item => options += `<option value="${item.id}">${item.name}</option>`);
        } else if (selectedType === 'badge') {
            this.state.badges.forEach(item => options += `<option value="${item.id}">${item.name}</option>`);
        }
    ...
    this.populateSpotsList = () => {
        ...
        listElement.innerHTML = this.state.mapSpots.map(spot => {
            const item = this.getItemForSpot(spot);
            return `
                <div class="bg-gray-700 p-3 rounded-lg flex items-center justify-between">
                    <div>
                        <p class="font-semibold">${item ? item.name : 'Linked item not found'}</p>
                        <p class="text-xs text-gray-400">${spot.spotType.charAt(0).toUpperCase() + spot.spotType.slice(1)} Spot</p>
                    </div>
                    <button onclick="app.handleDeleteSpot('${spot.id}')" class="p-2 bg-red-500/20 text-red-400 rounded-md"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
                </div>
            `;
        }).join('');
        lucide.createIcons();
    ...
    this.handleSaveSpot = async (e) => {
        ...
        if (!lat || !lng) {
            return this.showModal('error', 'Location Missing', 'Please click on the map to select a location for the spot.');
        }
        ...
        try {
            ...
            this.showModal('success', 'Spot Saved!', 'The new QR spot has been added to the map.');
            form.reset();
            document.getElementById('coords-display').textContent = 'Click map to select location';
        } catch (error) {
    ...
    this.handleDeleteSpot = async (spotId) => {
        this.showModal('confirm', 'Delete Spot?', 'Are you sure you want to remove this spot from the map?', async () => {
            try {
                await deleteDoc(doc(this.fb.db, this.paths.mapSpots, spotId));
                this.showModal('success', 'Spot Deleted', 'The spot has been removed.');
            } catch (error) {
    ...
    this.openAdminMapPanel = () => {
        const content = `
            <div class="flex flex-col lg:flex-row lg:space-x-4 h-full">
                ...
                <div class="lg:w-1/3 flex flex-col">
                    <h3 class="font-semibold text-lg mb-2">Add/Edit Spot</h3>
        ...
        // Now, run the logic that makes the panel interactive
        this.initAdminMap();
        this.populateAdminSpotForm();
        this.populateSpotsList();

        document.querySelector('#add-spot-form select[name="spotType"]').addEventListener('change', () => {
            this.populateAdminSpotForm();
        });
        document.getElementById('add-spot-form').addEventListener('submit', this.handleSaveSpot.bind(this));
    };
    ...
    this.startScanner = () => {
        ...
        Html5Qrcode.getCameras().then(cameras => {
            ...
            statusEl.textContent = "Camera access is required. Please grant permission in your app settings.";
        });
    };
    ...
    this.fetchAllRewardsForAdmin = async () => {
        try {
            ...
            this.state.adminRewards = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    this.startAdminScanner = async () => {
        ...
        const qrCodeSuccessCallback = async (decodedText) => {
            ...
            const selectedBadgeId = document.getElementById('admin-badge-select-hidden').value;
            ...
            try {
                ...
                if (selectedRewardId) {
                    ...
                    if (reward) {
                        ...
                        await this.addPointLog(userId, `Admin award: ${reward.name}`, pointsChange, batch, beforePoints, afterPoints);
    ...
    this.handleAdminSearch = (type, term) => {
        const lowercasedTerm = term.toLowerCase(); if (type === 'member') this.state.adminMemberSearch = lowercasedTerm; if (type === 'reward') this.state.adminRewardSearch = lowercasedTerm; this.render();
    ...
    this.handleAdminBadgeSelect = (badgeId) => {
        ...
        const previewElement = document.getElementById('admin-badge-selector-preview');
    ...
    this.renderThemeOption = (themeId, themeName, bgClass, activeTheme) => {
        ...
        return `
            <button onclick="app.handleThemeChange('${themeId}')" class="flex flex-col items-center space-y-2 p-2 rounded-lg ${isActive ? 'bg-gray-600' : 'bg-gray-700'}">
                <div class="w-8 h-8 rounded-full ${bgClass} ${isActive ? 'ring-2 ring-pink-500 ring-offset-2 ring-offset-gray-700' : ''}"></div>
